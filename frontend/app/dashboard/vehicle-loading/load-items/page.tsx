...
export default function LoadItemsPage() {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (!target.closest('.product-name-input')) {
  ...
  const searchInventoryItems = async (query: string) => {
    ...
    try {
      const response = await axios.get(`http://localhost:8000/api/stock/inventory?search=${encodeURIComponent(query)}&status=active`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });

      // Handle different response formats from inventory API
      ...
      setInventorySuggestions(items.slice(0, 10)); // Limit to 10 suggestions
  ...
  const selectInventoryItem = (item: InventoryItem) => {
    setFormData({
      ...
      sell_price: item.unit_price.toString() // Using same price for both, can be adjusted
    });
    setShowSuggestions(false);
  };
              ...
              <div className="mt-3">
                ...
                <form onSubmit={handleSubmit}>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="relative product-name-input">
                      <label className="block text-sm font-medium text-gray-700 mb-2">
                        Product Name *
                      </label>
                      <input
                        ...
                        onFocus={() => formData.name && setShowSuggestions(inventorySuggestions.length > 0)}
                        className="w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-green-500 focus:border-green-500 text-gray-900"
                        ...
                        <div className="absolute z-10 w-full bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-y-auto mt-1">
                          {inventorySuggestions.map((item) => (
                            ...
                            >
                              ...
                              <div className="text-sm text-gray-600">
                                Code: {item.code} | Stock: {item.current_stock} {item.unit} | Price: LKR {item.unit_price}
                              </div>
                              <div className="text-xs text-gray-500">
                                Type: {item.type === 'finished_good' ? 'Finished Product' : 'Raw Material'}
                              </div>
                            </div>
                          ))}
                        </div>
