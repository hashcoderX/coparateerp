...
export default function DistributionCollectionReportPage() {
  ...
  useEffect(() => {
    ...
    const verifyAccess = async () => {
      try {
        ...
        console.error('Error checking collection report access:', error);
        router.push('/dashboard');
      } finally {
        setAccessReady(true);
      }
  ...
  const fetchRows = async (authToken?: string) => {
    ...
    try {
      ...
      console.error('Error fetching collection report rows:', error);
      setRows([]);
      setErrorMessage('Failed to load collection report data.');
    } finally {
      setLoading(false);
    }
  };
  ...
  }, [rows, search, statusFilter, methodFilter, customerFilter, fromDate, toDate]);

  const summary = useMemo(() => {
    const totalCollections = filteredRows.reduce((sum, row) => sum + parseAmount(row.amount), 0);
    ...
    const statusCounts = {
      received: filteredRows.filter((row) => row.status === 'received').length,
      cleared: filteredRows.filter((row) => row.status === 'cleared').length,
      bounced: filteredRows.filter((row) => row.status === 'bounced').length,
  ...
  const exportCsv = () => {
    ...
    link.href = url;
    link.download = `distribution-collection-report-${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const buildPdf = () => {
    ...
    doc.setFontSize(16);
    doc.text('Distribution Collection Report', 40, 40);

    doc.setFontSize(10);
    doc.text(`Generated: ${new Date().toLocaleString()}`, 40, 58);
    doc.text(`Status Filter: ${statusFilter === 'all' ? 'ALL' : statusFilter.toUpperCase()}`, 40, 72);
    doc.text(`Method Filter: ${methodFilter === 'all' ? 'ALL' : paymentMethodLabel(methodFilter)}`, 40, 86);
    ...
    autoTable(doc, {
      ...
      didDrawPage: () => {
        doc.setFontSize(9);
        doc.text(
          `Payments: ${summary.totalPayments} | Total: ${formatMoney(summary.totalCollections)} | Cash: ${formatMoney(summary.methodTotals.cash)} | Check: ${formatMoney(summary.methodTotals.check)} | Bank Transfer: ${formatMoney(summary.methodTotals.bank_transfer)}`,
          20,
  ...
  const downloadPdf = () => {
    const doc = buildPdf();
    doc.save(`distribution-collection-report-${new Date().toISOString().split('T')[0]}.pdf`);
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row justify-between sm:items-center h-auto sm:h-16 py-3 sm:py-0 gap-3 sm:gap-0">
            ...
            <div className="hidden md:flex items-center space-x-2 text-sm text-gray-600">
              ...
              <span>Collection Report Ready</span>
            </div>
          </div>
        </div>
      </nav>
      ...
      <main className="relative z-10 max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-8">
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-2">
            Collection <span className="bg-gradient-to-r from-green-600 to-teal-600 bg-clip-text text-transparent">Report</span>
          </h1>
          <p className="text-gray-600">Collections by customer, payment method and status with export-ready insights.</p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-6 gap-3">
            <div className="md:col-span-2">
              ...
              <input
                ...
                placeholder="Payment no, invoice no, customer, ref"
        ...
        <section className="rounded-2xl border border-white/70 bg-white/90 backdrop-blur-lg shadow-xl overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-100 bg-gradient-to-r from-green-50 to-teal-50 flex flex-wrap gap-3 text-sm text-black">
            ...
            <span>Total Collection: <strong>{formatMoney(summary.totalCollections)}</strong></span>
            ...
            <span>Cleared: <strong>{summary.statusCounts.cleared}</strong></span>
          </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-100">
                {filteredRows.length === 0 ? (
                  <tr>
                    <td colSpan={9} className="px-4 py-10 text-center text-sm text-gray-500">No collection records found.</td>
                  </tr>
                ) : (
